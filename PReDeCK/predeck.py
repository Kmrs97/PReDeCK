...
##Materialized PartOf Facts generated thorugh the inference phase
...
partOf("Cap","Aeroplane","Artifact").
 ...
 '''   
    ...
    isPartOf(B1,Lbl1,B2,Lbl2):-candidatePartOf(B1,B2),
                    partBox(B1,Lbl1,C),
                    objectBox(B2,Lbl2,C),
                    partOf(Lbl1,Lbl2,C).
  ...
  # Rules to separate objects and parts
    ...
    if path.exists(opath)==False:
        os.mkdir(opath) 
    with open(opath+"/predeck_inferences_JSON.json", "w") as outfile:
